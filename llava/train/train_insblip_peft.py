...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        visual_encoder = vision_tower.to(dtype=dtype, device=training_args.device)   
        ...
        #llm_proj_weight = q_former_state_dict['opt_proj.weight']
        #llm_proj_bias = q_former_state_dict['opt_proj.bias']
        query_tokens = torch.nn.Parameter(q_former_state_dict['query_tokens'].to(dtype=dtype, device=training_args.device))
        ...
        #del q_former_state_dict['opt_proj.weight']
        #del q_former_state_dict['opt_proj.bias']
        ...
        Qformer.load_state_dict(q_former_state_dict)
        print("\nLoaded pretrained Qformer weights")
        ...
        llama_proj = llama_proj.to(dtype=dtype, device=training_args.device)

        model.model.llama_proj = llama_proj #.to(dtype=torch.float16)
    ...
    config = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        modules_to_save=['Qformer','query_tokens','llama_proj']   ## 'Qformer', , 'query_tokens'
    )
    ...
    model.save_pretrained('save_pretrained/trained_mixed_insblip_epoch1')

if __name__ == "__main__":
    print("start training peft insblip")
    train()